...
def main():
    ...
    pitch = 12  # Space between holes
    margin_x = 24
    margin_y = 24
    
    # 400 point board: 30 rows, 5 columns (a-e), gap, 5 columns (f-j).
    # Power rails: 2 columns on left, 2 columns on right. 
    # Usually power rails have 5 blocks of 5 holes = 25 holes.
    # Total width:
    # L_power: 2 cols
    # gap: 1 col
    # A-E: 5 cols
    # Center Gap: 3 cols (for DIP chips)
    # F-J: 5 cols
    # gap: 1 col
    # R_power: 2 cols
    # Total cols = 2 + 1 + 5 + 3 + 5 + 1 + 2 = 19 columns
    
    # Total rows = 30 for main terminal strips.
    # Power strips often have 25 holes, spaced slightly differently or aligned with every 5 rows.
    # Let's align power strips with rows 2,3,4,5,6 ; gap ; 8,9,10,11,12 ... etc.
    # Actually, simpler to just map 25 holes linearly matching row height (skip every 6th).
    ...
    for block in range(5):
        for h in range(5):
            row_idx = block * 6 + h + 1 # aligns roughly with 30 rows
    ...
    # Draw center divider groove
    x_center = margin_x + 8.5 * pitch
    svg += f'  <rect x="{x_center - 3}" y="{margin_y}" width="6" height="{height - 2*margin_y}" fill="#e8e8e8" rx="2"/>\n'

    # Draw Terminal Strips A-E and F-J
    # Left block starts at index 3 (0=L-, 1=L+, 2=gap)
    # Right block starts at index 11 (3+5=8. 8..10=center. 11..15)
    labels_left = ['a','b','c','d','e']
    labels_right = ['f','g','h','i','j']
    
    for r in range(30):
        y = margin_y + r * pitch
        # Number labels every 5 rows
        if r % 5 == 0 or r == 29:
    ...
    with open('public/assets/components/breadboard-400.svg', 'w') as f:
        f.write(svg)
        
    print(f"SVG saved. Width: {width}, Height: {height}, Pins count: {len(pins)}")
    
    # Save pin dictionary for insertion into componentSvgCatalog.js
    import json
    js_snippet = f"'breadboard-400': {{\n  width: {width},\n  height: {height},\n  pins: {json.dumps(pins, indent=2)}\n}}"
    with open('breadboard-400-pins.txt', 'w') as f:
        f.write(js_snippet)
    print("Pin mapping written to breadboard-400-pins.txt")
