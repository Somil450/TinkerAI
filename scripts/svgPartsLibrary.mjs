...
/** Blue PCB breakout with labeled header */
export function blueBreakout(w, h, label, pinLabels, opts = {}) {
  const uid = opts.uid || 'bb'
  const headerW = pinLabels.length * 12 + 16
  const hx = (w - headerW) / 2
  let body = svgOpen(w, h, uid)
  body += `<rect x="3" y="18" width="${w - 6}" height="${h - 22}" rx="4" fill="url(#pcb-${uid})" stroke="#0a3d7a" stroke-width="1.5" filter="url(#sh-${uid})"/>`
  body += `<rect x="${hx}" y="4" width="${headerW}" height="16" rx="2" fill="#1a1a1a" stroke="#333"/>`
  pinLabels.forEach((lb, i) => {
    const px = hx + 8 + i * 12
    body += `<rect x="${px - 3}" y="7" width="6" height="10" fill="url(#pin-${uid})" rx="0.5"/>`
    body += `<text x="${px}" y="3" text-anchor="middle" fill="#fff" font-size="6" font-weight="bold" font-family="Arial,sans-serif">${lb}</text>`
  })
  if (opts.chip) {
...
export function arduinoNano() {
  ...
  s += `<rect x="6" y="12" width="188" height="56" rx="4" fill="url(#pcbTeal-nano)" stroke="#00a8b0" filter="url(#sh-nano)"/>`
  ...
  for (let i = 0; i < 15; i++) {
    s += `<rect x="${18 + i * 11}" y="7" width="5" height="6" fill="url(#pin-nano)"/>`
    s += `<rect x="${18 + i * 11}" y="67" width="5" height="6" fill="url(#pin-nano)"/>`
  }
...
export function arduinoMega() {
  ...
  s += `<path d="M 10 10 L 330 10 A 15 15 0 0 1 345 25 L 345 200 A 15 15 0 0 1 330 215 L 25 215 A 15 15 0 0 1 10 200 L 10 25 A 15 15 0 0 1 10 10 Z" fill="url(#pcbTeal-mega)" stroke="#00a8b0" stroke-width="2" filter="url(#sh-mega)"/>`
  ...
  for (let i = 0; i < 28; i++) s += `<rect x="${68 + i * 9.5}" y="9" width="5" height="10" fill="url(#pin-mega)"/>`
  for (let i = 0; i < 16; i++) s += `<rect x="${68 + i * 9.5}" y="201" width="5" height="10" fill="url(#pin-mega)"/>`
...
export function esp32Devkit() {
  ...
  s += `<rect x="4" y="8" width="272" height="84" rx="4" fill="#111" stroke="#333" filter="url(#sh-esp)"/>`
...
export function hcSr04() {
  ...
  s += `<rect x="4" y="22" width="122" height="58" rx="4" fill="url(#pcb-sr04)" stroke="#0a3d7a" filter="url(#sh-sr04)"/>`
  const eye = (cx) => `
  ...
  s += eye(38) + eye(92)
  s += `<rect x="18" y="4" width="94" height="18" rx="2" fill="#1a1a1a"/>`
  ;['VCC', 'TRIG', 'ECHO', 'GND'].forEach((lb, i) => {
    const px = 28 + i * 22
    s += `<rect x="${px - 4}" y="8" width="8" height="10" fill="url(#pin-sr04)"/>`
    s += `<text x="${px}" y="6" text-anchor="middle" fill="#fff" font-size="7" font-weight="bold">${lb}</text>`
  })
  s += `<text x="65" y="78" text-anchor="middle" fill="#fff" font-size="9" font-weight="bold">HC-SR04</text>`
  return s + svgClose()
}
...
export function dht22Module() {
  ...
  <rect x="10" y="8" width="60" height="34" rx="3" fill="url(#dhtPcb)" stroke="#0D47A1"/>
  <rect x="12" y="4" width="56" height="12" fill="#111"/>
  <text x="19" y="3" font-size="6" fill="#fff" font-weight="bold">VCC</text><text x="33" y="3" font-size="6" fill="#fff" font-weight="bold">DAT</text><text x="47" y="3" font-size="6" fill="#fff" font-weight="bold">NC</text><text x="61" y="3" font-size="6" fill="#fff" font-weight="bold">GND</text>
  <g fill="#ddd"><rect x="16" y="6" width="5" height="8"/><rect x="30" y="6" width="5" height="8"/><rect x="44" y="6" width="5" height="8"/><rect x="58" y="6" width="5" height="8"/></g>
...
export function pirSensor() {
  ...
  s += `<rect x="20" y="55" width="80" height="38" rx="4" fill="url(#pcb-pir)" stroke="#0a3d7a" filter="url(#sh-pir)"/>`
  ...
  ;['VCC', 'OUT', 'GND'].forEach((lb, i) => {
    const px = 38 + i * 22
    s += `<rect x="${px}" y="52" width="6" height="8" fill="url(#pin-pir)"/>`
    s += `<text x="${px + 3}" y="50" text-anchor="middle" fill="#fff" font-size="6" font-weight="bold">${lb}</text>`
  })
...
export function mq2Gas() {
  ...
  ;['VCC', 'GND', 'AO', 'DO'].forEach((lb, i) => {
    const px = 38 + i * 18
    s += `<rect x="${px}" y="48" width="6" height="8" fill="url(#pin-mq2)"/>`
    s += `<text x="${px + 3}" y="46" text-anchor="middle" fill="#fff" font-size="6" font-weight="bold">${lb}</text>`
  })
...
export function ldrSensor() {
  ...
  s += `<rect x="15" y="50" width="70" height="32" rx="3" fill="url(#pcb-ldr)" stroke="#0a3d7a"/>`
  ...
  ;['VCC', 'GND', 'OUT'].forEach((lb, i) => {
    const px = 28 + i * 22
    s += `<rect x="${px}" y="52" width="6" height="8" fill="url(#pin-ldr)"/>`
    s += `<text x="${px + 3}" y="50" text-anchor="middle" fill="#fff" font-size="6" font-weight="bold">${lb}</text>`
  })
  return s + svgClose()
}
...
export function soilMoisture() {
  ...
  s += `<rect x="20" y="8" width="50" height="28" rx="3" fill="url(#pcb-soil)" stroke="#0a3d7a"/>`
  ;['VCC', 'GND', 'SIG'].forEach((lb, i) => {
    const px = 28 + i * 16
    s += `<rect x="${px}" y="10" width="5" height="8" fill="#ddd"/>`
    s += `<text x="${px + 2}" y="8" text-anchor="middle" fill="#fff" font-size="5" font-weight="bold">${lb}</text>`
  })
...
export function lcd1602() {
  ...
  s += `<rect x="4" y="12" width="192" height="72" rx="4" fill="#2E7D32" stroke="#1B5E20" filter="url(#sh-lcd)"/>`
  ...
  s += `<rect x="10" y="6" width="180" height="8" fill="#111"/>`
  for (let i = 0; i < 16; i++) s += `<rect x="${14 + i * 11}" y="7" width="4" height="6" fill="#ddd"/>`
...
export function servoSg90() {
  ...
  s += `<rect x="8" y="35" width="72" height="48" rx="6" fill="#1565C0" stroke="#0D47A1" filter="url(#sh-srv)"/>`
  ...
  ;['SIG', '5V', 'GND'].forEach((lb, i) => {
    s += `<rect x="4" y="${38 + i * 14}" width="6" height="8" fill="#ddd"/>`
    s += `<text x="1" y="${44 + i * 14}" text-anchor="end" fill="#333" font-size="6" font-weight="bold">${lb}</text>`
  })
  return s + svgClose()
}
...
export function l298nDriver() {
  ...
  s += `<rect x="4" y="10" width="142" height="82" rx="4" fill="#1565C0" stroke="#0D47A1" filter="url(#sh-l298)"/>`
  ...
  s += `<rect x="8" y="4" width="134" height="8" fill="#111"/>`
